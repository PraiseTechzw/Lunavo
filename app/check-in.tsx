...
const moods = [
  { id: 'terrible', icon: 'sentiment-very-dissatisfied', label: 'Awful', color: '#EF4444' },
  { id: 'bad', icon: 'sentiment-dissatisfied', label: 'Down', color: '#F97316' },
  { id: 'okay', icon: 'sentiment-neutral', label: 'Fine', color: '#FBBF24' },
  { id: 'good', icon: 'sentiment-satisfied', label: 'Good', color: '#84CC16' },
  { id: 'awesome', icon: 'sentiment-very-satisfied', label: 'Great', color: '#10B981' },
];
...
const analyzeWellbeing = (mood: string, text: string) => {
  ...
  if (mood === 'terrible' || mood === 'bad') {
    if (note.includes('lonely') || note.includes('alone')) return { title: "You are not alone", message: "Loneliness is a heavy feeling. The Peer Chat is open if you want to connect anonymously.", action: '/(tabs)/chat', actionLabel: 'Go to Chat' };
    if (note.includes('stress') || note.includes('exam') || note.includes('work')) return { title: "Breathe Deeply", message: "Academic pressure is real. Remember to take small breaks. You've got this.", action: '/(tabs)/resources', actionLabel: 'View Study Tips' };
    return { title: "We hear you", message: "It's okay to have hard days. Be gentle with yourself today.", action: '/urgent-support', actionLabel: 'Support Options' };
  }

  if (mood === 'awesome' || mood === 'good') {
    return { title: "Keep Shining!", message: "Love this energy! Consider sharing a positive thought in the Community Forum.", action: '/create-post', actionLabel: 'Share Positivity' };
  }

  return { title: "Daily Checked", message: "Thanks for checking in! Consistent tracking helps build mindfulness.", action: '/(tabs)/resources', actionLabel: 'Explore Vault' };
};
    ...
    <ThemedView style={styles.container}>
            ...
            <View style={styles.glassInputContainer}>
              <View style={styles.labelRow}>
                ...
                <View style={styles.aiBadge}>
                  ...
                  <ThemedText style={{ fontSize: 10, color: '#FFF', fontWeight: 'bold' }}>AI Enabled</ThemedText>
                </View>
              </View>
          ...
          <View style={styles.footer}>
            <TouchableOpacity
              ...
              style={[
                ...
                (!selectedMood || isSubmitting) && { opacity: 0.5, transform: [{ scale: 0.95 }] }
              ]}
            >
              <LinearGradient
              ...
              >
                {isSubmitting ? (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    ...
                    <ThemedText style={styles.saveText}>Analyzing Wellness...</ThemedText>
                  </View>
                ) : (
                  <>
      ...
      <Modal visible={showInsight} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <Animated.View entering={ZoomIn.duration(400)} style={[styles.modalCard, { backgroundColor: colors.card }]}>
            <LinearGradient
            ...
            <View style={styles.modalContent}>
              <ThemedText type="h2" style={{ textAlign: 'center', marginBottom: 10 }}>
                {aiInsight?.title || "Care Insight"}
              </ThemedText>
              <ThemedText style={{ textAlign: 'center', opacity: 0.8, lineHeight: 22, marginBottom: 20 }}>
                {aiInsight?.message}
              </ThemedText>
              ...
              >
                <ThemedText style={{ fontSize: 12, opacity: 0.5 }}>Dismiss</ThemedText>
              </TouchableOpacity>
            </View>
          </Animated.View>
        </View>
...
const styles = StyleSheet.create({
  ...
  closeBtn: { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  ...
  moodGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 12, marginBottom: Spacing.xl },
  moodCard: { borderRadius: BorderRadius.xxl, justifyContent: 'center', alignItems: 'center', gap: 8 },
  moodLabel: { fontSize: 12, marginTop: 4 },
  section: { marginTop: Spacing.md },
  glassInputContainer: { backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: BorderRadius.xl, padding: Spacing.lg, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  labelRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, marginBottom: Spacing.md },
  input: { height: 120, textAlignVertical: 'top', fontSize: 16, lineHeight: 24 },
  privacyBanner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 4, marginTop: 12 },
  privacyText: { fontSize: 12, opacity: 0.6 },
  footer: { marginTop: 40, marginBottom: 60 },
  saveBtnWrapper: { ...PlatformStyles.premiumShadow },
  saveBtn: { paddingVertical: 18, paddingHorizontal: Spacing.xl, borderRadius: BorderRadius.full, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: Spacing.md },
  saveText: { color: '#FFF', fontWeight: '800', fontSize: 16, letterSpacing: 0.5 },
  aiBadge: { marginLeft: 'auto', backgroundColor: '#8B5CF6', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 4 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 20 },
  modalCard: { width: '100%', maxWidth: 320, borderRadius: 24, overflow: 'hidden', alignItems: 'center' },
  modalHeader: { width: '100%', height: 100, justifyContent: 'center', alignItems: 'center' },
  modalContent: { padding: 24, alignItems: 'center', width: '100%' },
  modalBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12 },
});
