...
export function mapResourceFromDB(data: any): Resource & { approved?: boolean; sourceType?: string } {
  ...
  // Use thumbnail URL from database if available
  // For videos, this will be the uploaded thumbnail generated by expo-video-thumbnails
  let thumbnailUrl = data.thumbnail_url;
  
  if (!thumbnailUrl) {
    // Auto-generate thumbnail URLs based on actual resource type (fallback only)
    if (actualResourceType === 'image' || actualResourceType === 'infographic') {
      // For images/infographics, use the file path or URL as thumbnail
      thumbnailUrl = data.file_path || data.url;
    } else if (actualResourceType === 'video' || actualResourceType === 'short-video') {
      // For videos without uploaded thumbnail, don't auto-generate
      // This ensures we use icons instead of trying to load video files as images
      // The thumbnail should be uploaded when creating the resource
